...
In client components, dynamic params can be accessed using the `use` hook, or the `useParams` hook.
...
export default function BlogPostPage({
  ...
  const fromProps = use(params)
  const fromHook = useParams()
...
export default async function Page({
  ...
  // validate that `from` can be parsed into a valid date

  // query your database for records `from`
...
Verify that the incoming request has [access rights](/docs/app/guides/authentication) to these records.

### `useSearchParams`

In a [**dynamically rendered**](/docs/app/getting-started/partial-prerendering#dynamic-rendering) page, Client Components can use the [`useSearchParams`](/docs/app/api-reference/functions/use-search-params) hook to read search parameters, without being opted-out from being server-side rendered. Learn more about [Client Components server side rendering](/docs/app/getting-started/server-and-client-components#on-the-server).

In development, pages are rendered on-demand, this gives `useSearchParams` access to search params in the incoming request. This does not reflect production behavior for [static pages](/docs/app/getting-started/partial-prerendering#static-rendering), which are pre-rendered without a request.

Accessing `useSearchParams` during pre-rendering will cause the Client Component tree up to the nearest Suspense boundary, to be client-side rendered. If there’s no such boundary pre-rendering fails.

### What to use and when

- Use the `searchParams` prop when you need search parameters to **load data for the page**  
...
- As a small optimization, you can use `new URLSearchParams(window.location.search)` in **callbacks or event handlers**  
